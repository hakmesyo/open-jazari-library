...
//        writeStatistics(gr, w, h);
        ...
        Point[] mp = mappingDataToScreenCoordinates(cm.toFloatArray2D(), w, h, px, py);
    ...
    private void writeStatistics(Graphics gr, int w, int h) {
        ...
        ArrayList<Float> metricVal = this.figureAttribute.perfMetricVal;
        int delta = 25;
        int py = 5;
        int px = w - 140;
    ...
    private void drawYAxis(Graphics2D gr, int x0, int y0, int w, float[][] d) {
        float maxY = getMaxYValue(d[1]);
        float minY = getMinYValue(d[1]);
...
//        int l = y0 - 60;
    ...
    private void drawXAxis(Graphics2D gr, int x0, int y0, int h, int w, float[][] d) {
        float maxX = getMaxXValue(d[0]);
        float minX = getMinXValue(d[0]);
...
//        gr.drawLine(x0, y0, w - m, y0);
    ...
    private Point[] mappingDataToScreenCoordinates(float[][] d, int w, int h, int px, int py) {
    ...
    private float getMinXValue(float[] d) {
        float ret = 0;
        ret = d[0];
        for (int i = 0; i < d.length; i++) {
            if (ret > d[i]) {
