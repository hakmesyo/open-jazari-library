...
public class FactoryColorMap {
     ...
     */
    public static Color[] mapColorDefault(float[] d){
        Color[] ret=new Color[d.length];
        float[] n=FactoryNormalization.normalizeIntensity(d, -1,1);
     ...
     * Generate default color map. Color ranges from blue(darkest) to red color. 
     * At the center green color is positioned
     * @param d2 : float[][] array
     * @return : 1D Color Array
     */
    public static Color[][] mapColorDefault(float[][] d2){
        int nr=d2.length;
        int nc=d2[0].length;
        float[] d=FactoryMatrix.concatenateRows(d2);
        Color[] ret=new Color[d.length];
        float[] n=FactoryNormalization.normalizeIntensity(d, -1,1);
//        System.out.println(Arrays.toString(n));
        for (int i = 0; i < n.length; i++) {
     ...
     * @return : 1D Color Array
     */
    public static Color[][] mapColorAroundCenterColor(float[][] d2,Color centeredColor){
        int nr=d2.length;
        int nc=d2[0].length;
        float[] d=FactoryMatrix.concatenateRows(d2);
        Color[] ret=new Color[d.length];
        float[] n=FactoryNormalization.normalizeIntensity(d,0,1);
//        System.out.println(Arrays.toString(n));
        float r=centeredColor.getRed();        
        float g=centeredColor.getGreen();
        float b=centeredColor.getBlue();
        
        float d_up_red=255-r;
        float d_up_green=255-g;
        float d_up_blue=255-b;
