...
public class Welcome2 extends Applet 
	...
	public void create_net(float[] parameters) {
		System.out.print("\nCreate_net() ");
	...
	public void start_training(float[][] inputs, float[][] outputs) {
...
class ConfigFrame extends MyFrame implements ActionListener {
	...
	public ConfigFrame( Welcome2 a ) {
		...
		addComponent( new Label(
			"Following parameters must in float format",
	...
	public void update_parameters() {
		...
		for (int i=0; i < INT_INPUTS; i++) {
			inputs[i] = -1.0f;
			inputs[i] = 
				(float) Integer.parseInt( (fields[i]).getText() );
...
class ErrorFrame extends MyFrame {
	...
	public void new_point( float point ) {
	...
	public void set_epoch_limit(int max_epoch) {
		...
		if ( (float) epoch_gap < 
			(float)epoch_limit/(float)c.getWidth() ) {
			epoch_gap++;
		} // if
...
class ErrorPlot extends Canvas {
	...
	public void plot(int epoch, float error) {
		if (error > max_error) { max_error = 1.1f*error; }
...
class NetFrame extends MyFrame {
	...
	"may be of float format only .. otherwise no change occurs, " +
	...
	*/
	...
	public long get_delay() {
		...
		// if the text entered isn't a float number,
		// then restore the old time and show it in the box.
		catch(NumberFormatException _e) {
			time.setText("" + (float) (delay/1000) );
			return delay; 
		} // catch()
...
class NetPic {
	...
	public void draw( Graphics g ) {
		...
		for (int i=1; i < node_layers.length; i++) {
			...
			for (int j=0; j < nodes_curr.length; j++) {
				float[] weights = ((node_layers[i]).get_nodes())[j].get_weights();
	...
	// dummy so far .. 
	public Color get_colour(float in) {
...
class LayerPic {
	...
	public void draw( Graphics g, float[] outputs ) {
	...
	public Color get_colour(float output) {
...
class ClickCanvas extends Canvas implements MouseListener {
	...
	public float[] get_inputs() {
		float[] inputs = new float[s.length];

		for (int i=0; i < s.length; i++) {
			if (s[i].isClicked()) { inputs[i] = 1.0f; }
			else { inputs[i] = 0.0f; }
		} // for
...
class ClickFrame extends MyFrame implements ActionListener {
	...
	// final float[DEFAULT_SAMPLES][DEFAULT_INPUTS] default_input
	...
	float[][] inputs = new float[DEFAULT_SAMPLES][DEFAULT_INPUTS];
	...
	public void actionPerformed( ActionEvent e ) {
		...
		if (( e.getSource() == b ) && ( train_mode == true )) {
			if (current_sample != max_samples) { 
				...
				float[][] encoded_outputs =
					new float[max_samples][outputs_count];

				for (int i=0; i < max_samples; i++) {
					for (int j=0; j < outputs_count; j++) {
						if (i == j) {
							encoded_outputs[i][j] = 1.0f;
						} // if
						else {
							encoded_outputs[i][j] = 0.0f;
						} // else
					} // for
				} // for
