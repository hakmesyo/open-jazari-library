package jazari.machine_learning.classifiers.anten_nd4j_cnn;
...
import jazari.machine_learning.classifiers.anten_nd4j_cnn.matrix.MatrixElement;
    ...
    private FeedForwardResult[] feedForward(Matrix x, Matrix[] dropoutMasks) {
        ...
        for (int layer = 1; layer < numLayers; layer++) {
            ...
            if (myLayerParams[layer].isConvolutional()) {
                ...
                Matrix temp=myThetas[layer].copy();                
                ffr[layer].output = ffr[layer].input.trans2mult(temp);
