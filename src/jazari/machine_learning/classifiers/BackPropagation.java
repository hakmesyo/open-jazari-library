...
public class BackPropagation extends Thread {
    ...
    private void CalculateOverallError() {
        ...
        for (i = 0; i < NumberOfSamples; i++) {
            for (j = 0; j < Layer[NumberOfLayers - 1].Node.length; j++) {
                OverallError
                        = OverallError
                        + 0.5f * (float)(Math.pow(ExpectedOutput[i][j]
                                - ActualOutput[i][j], 2));
            }
        }
    }
    ...
    public BackPropagation(int NumberOfNodes[],
        ...
        Input = new float[NumberOfSamples][Layer[0].Node.length];
        ExpectedOutput = new float[NumberOfSamples][Layer[NumberOfLayers - 1].Node.length];
        ActualOutput = new float[NumberOfSamples][Layer[NumberOfLayers - 1].Node.length];
