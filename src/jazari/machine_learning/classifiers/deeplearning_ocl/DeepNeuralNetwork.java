...
//    }
    ...
    public void train(Matrix x, Matrix y) throws Exception {
        ...
        if (params.normalizeNumericData) {
            ...
            x = x.copy();
            averages = new float[x.numColumns()];
            stdDevs = new float[x.numColumns()];
...
//       myParams.numThreads = myParams.numThreads > 0 ? myParams.numThreads : 20;
        ...
        float cost = getCostThreaded(batchesX, batchesY, params.weightPenalty);
        LinkedList<Float> fiveLastCosts = new LinkedList<>();
        LinkedList<Float> tenLastCosts = new LinkedList<>();
        if (params.debug) {
        ...
        for (int i = 0; i < params.maxIterations && !halted; i++) {
            ...
            if (fiveLastCosts.size() == 5) {
                ...
                float oldCost = fiveLastCosts.remove();
                float minCost = (float)Math.min(cost, Collections.min(fiveLastCosts));
                if (minCost >= oldCost) {
                    params.learningRate = params.learningRate * 0.1f;
                    fiveLastCosts.clear();
                }
            }
            if (tenLastCosts.size() == 10) {
                float minCost = Math.min(cost, Collections.min(tenLastCosts));
                float maxCost = Math.max(cost, Collections.max(tenLastCosts));
                tenLastCosts.remove();
...
//    public float[] getPredictions(float[] x) {
//        Matrix xMatrix = new Matrix(new float[][]{x});
...
//    }
    private Matrix createTheta(int rows, int cols) {
        ...
        float epsilon = (float)(Math.sqrt(6) / Math.sqrt(rows + cols));
        ...
        for (int layer = 1; layer < numLayers; layer++) {
            ...
            if (layerParams[layer].isConvolutional()) {
                ...
                if (dropoutMasks != null && dropoutMasks[layer - 1] != null) {
                    ...
                    float dropoutRate = layer - 1 == 0 ? params.inputLayerDropoutRate : params.hiddenLayersDropoutRate;
                    if (dropoutRate > 0.0) {
                        // Adjust values if training was done with dropout.
                        ffr[layer - 1].output.scale(1.0f - dropoutRate);
                    }
                }
...
//        Matrix mm=ffr[2].input;
    ...
    private float getCost(Matrix x, Matrix y, float weightPenalty, int batchSize) {
        float c = 0.0f;
        ...
        if (softmax) {
            ...
            // sum = sum(sum(t1))
            float[][] d = t1.data.toFloatArray2D();
            int r = d.length;
            int col = d[0].length;
            for (int i = 0; i < r; i++) {
                for (int j = 0; j < col; j++) {
...
//                }
    ...
    private float getCostThreaded(List<Matrix> batchesX, List<Matrix> batchesY, final float weightPenalty) throws Exception {
        final int batchSize = batchesX.get(0).numRows();
        // Queue up cost calculation in thread pool
        List<Future<Float>> costJobs = new ArrayList<>();
        for (int batchNr = 0; batchNr < batchesX.size(); batchNr++) {
            final Matrix bx = batchesX.get(batchNr);
            final Matrix by = batchesY.get(batchNr);
            Callable<Float> costCalculator = new Callable<Float>() {
                public Float call() throws Exception {
                    thetasLock.readLock().lock();
                    float cost = getCost(bx, by, weightPenalty, batchSize);
    ...
    private Matrix[] getGradients(Matrix x, Matrix y, float weightPenalty, Matrix[] dropoutMasks, int batchSize) {
...
//        }
        ...
        for (int layer = 1; layer < numLayers; layer++) {
            ...
            // Have to use batch size and not number of rows, in case that the last batch contains fewer examples.
            thetaGrad[layer].scale(1.0f / batchSize);
        }
            ...
            for (int thetaNr = 1; thetaNr < numLayers; thetaNr++) {
                ...
                for (int row = 0; row < grad.numRows(); row++) {
                    for (int col = 1; col < grad.numColumns(); col++) {
                        float regTerm = weightPenalty / numNodes * Math.signum(theta.get(row, col));
        ...
        for (int l = 0; l < masks.length; l++) {
            ...
            if (!layerParams[l + 1].isConvolutional()) {
                if ((l == 0 && params.inputLayerDropoutRate > 0) || (l > 0 && params.hiddenLayersDropoutRate > 0)) {
                    ...
                    float dropoutRate = l == 0 ? params.inputLayerDropoutRate : params.hiddenLayersDropoutRate;
                    float[][] d=mask.data.toFloatArray2D();
                    int r=d.length;
                    int c=d[0].length;
                    for (int i = 0; i < r; i++) {
                        for (int j = 0; j < c; j++) {
                            d[i][j]=(float)rnd.nextDouble() > dropoutRate ? 1.0f : 0.0f;
                        }
                    }
...
//                    }
    ...
    private void trainOneIterationThreaded(List<Matrix> batchesX, List<Matrix> batchesY, final float learningRate, final float weightPenalty) throws Exception {
        final int batchSize = batchesX.get(0).numRows();
    ...
    private float findInitialLearningRate(Matrix x, Matrix y, int batchSize, float weightPenalty, boolean debug) throws Exception {
