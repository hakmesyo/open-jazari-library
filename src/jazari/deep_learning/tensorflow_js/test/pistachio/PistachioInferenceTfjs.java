 ...
 */
package jazari.deep_learning.tensorflow_js.test.pistachio;
 ...
 * @author cezerilab
 */
public class PistachioInferenceTfjs {
...
//        new TestInferenceAsCMatrixApproach().generateInference(5,8084,8891,0,files.length-1);
    ...
    private static void newThread(int ch, int http_server_port, int web_socket_port, int from, int to) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                new PistachioInferenceTfjs().generateInference(ch, 8080 + ch, 8887 + ch, 0, files.length - 1);
            }
        }).start();
    }
    ...
    private void generateInference(int ch, int http_server_port, int web_socket_port, int from, int to) {
        ...
        for (int i = from; i < to; i++) {
            ...
            s = "./" + s.replace(new File(new File(FactoryUtils.currDir), "/models/pistachio_rest/").getAbsolutePath(), "");
