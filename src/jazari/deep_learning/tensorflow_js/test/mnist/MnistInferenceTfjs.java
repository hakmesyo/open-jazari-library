 ...
 */
package jazari.deep_learning.tensorflow_js.test.mnist;
 ...
 * @author cezerilab
 */
public class MnistInferenceTfjs {

    int channel = 0;
    long t1 = FactoryUtils.tic();
    private static String imgPath = FactoryUtils.getDefaultDirectory() + "/models/mnist_tfjs/images/test";
...
//        }
        ...
        new MnistInferenceTfjs().generateInference(1, 8080, 8887, 0, files.length-1);
...
//        new TestInferenceAsCMatrixApproach().generateInference(5,8084,8891,0,files.length-1);
    ...
    private void generateInference(int ch, int http_server_port, int web_socket_port, int from, int to) {
        ...
        CMatrix cm = CMatrix.getInstance()
                .setModelForInferenceTensorFlowJS(EnumRunTime.RUN, EnumOperatingSystem.WINDOWS, EnumEngine.TENSORFLOW_JS, "models/mnist_tfjs", this::invokeMethod, http_server_port, web_socket_port);
        ...
        try {
            ...
            Logger.getLogger(MnistInferenceTfjs.class.getName()).log(Level.SEVERE, null, ex);
        }

        for (int i = from; i < to; i++) {
            s = files[i].getAbsolutePath();
            s = "./" + s.replace(new File(new File(FactoryUtils.currDir),"/models/mnist_tfjs/").getAbsolutePath(), "");
