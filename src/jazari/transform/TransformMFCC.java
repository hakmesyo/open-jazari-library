...
public class TransformMFCC {   
     ...
     */   
    public TransformMFCC(int nnumberOfParameters,   
                float dsamplingFrequency,   
        ...
        calculateMelBasedFilterBank(dsamplingFrequency,   
                                    nnumberofFilters,   
                                    nFFTLength);   
        m_fft = new TransformFFT(m_nFFTLength); //initialize FFT   
        initializeDCTMatrix();   
        ...
        if (m_oisLifteringEnabled) {   
            ...
            float dfactor = m_nlifteringCoefficient / 2.0f;   
            float dfactor2 = (float)Math.PI / m_nlifteringCoefficient;   
            for (int i=0; i<m_nlifteringCoefficient; i++) {   
                m_nlifteringMultiplicationFactor[i] = 1.0f + dfactor * (float)Math.sin(dfactor2*(i+1));   
            }   
     ...
     */   
    public static float[] convertHzToMel(float[] dhzFrequencies, float dsamplingFrequency) {   
        float[] dmelFrequencies = new float[dhzFrequencies.length];   
        for (int k=0; k<dhzFrequencies.length; k++) {   
            dmelFrequencies[k] = 2595.0f*(float)( Math.log(1.0 + (dhzFrequencies[k] / 700.0) ) / Math.log(10) );   
        }   
     ...
     */   
    private void calculateMelBasedFilterBank(float dsamplingFrequency,   
                                             int nnumberofFilters,   
                                             int nfftLength) {   
        ...
        float ddeltaFrequency = dsamplingFrequency / nfftLength;   
        ...
        for(int i=1;i<=nnumberofFilters;i++) {   
            for(int j=m_nboundariesDFTBins[i-1][0],k=0;j<=m_nboundariesDFTBins[i-1][1];j++,k++) {   
                if (dfftFrequenciesInMel[j] < dfilterCenterFrequencies[i]) {   
                    ...
                    m_dweights[i-1][k] = 1.0f -(float)( (dfftFrequenciesInMel[j]-dfilterCenterFrequencies[i]) /   
    ...
    public float[] getFilterBankOutputs(float[] fspeechFrame) {   
        ...
        public static void main(String[] args) {   
          ...
          float dsamplingFrequency = 8000.0f;   
          int nFFTLength = 512;   
          ...
          TransformMFCC mfcc = new TransformMFCC(nnumberOfMFCCParameters,   
                               dsamplingFrequency,   
                               nnumberofFilters,   
                               nFFTLength,   
                               oisLifteringEnabled,   
          ...
          System.out.println("MFCC parameters:"+Arrays.toString(dparameters));   
